...
F = fft2(f,M,M);
z = rand(N) .* exp(1i * 2*pi * rand(N)); z = prior(z);
beta = 0.7;
counter = 0;
...
while counter < max_iter
    ...
    z = ifft2(Z); z = z(1:N,1:N);        % add "symmetric" as an input for ifft2 when working with real signals
    z = prior(z) + (1 - prior(ones(N))) .* (prev - beta * z);
